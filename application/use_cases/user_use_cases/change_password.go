...
func (u *UserUseCases) ChangePassword(ctx context.Context, password, newpassword string) (bool, *string, *string, error) {
	...
	if err != nil {
		return false, nil, nil, use_cases.ErrAccessTokenInvalid
	}
	...
	if err != nil {
		return false, nil, nil, use_cases.ErrDBFailure(err)
	}
	if err := bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(password)); err != nil {
		slog.Error("Failed comparing password", "err", err)
		return false, nil, nil, use_cases.ErrIncorrectPassword
	}
	...
	if err != nil {
		...
		return false, nil, nil, use_cases.ErrFailedToGeneratePasswordHash
	}
