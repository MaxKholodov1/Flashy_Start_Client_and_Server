...
class EditingDeckScreen extends StatefulWidget {
  const EditingDeckScreen({super.key});

  @override
  State<EditingDeckScreen> createState() => _EditingDeckScreenState();
}

class _EditingDeckScreenState extends State<EditingDeckScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<EditScreensCubit, EditingScreenState>(
      builder: (context, state) {
        if (state is! EditingDeckScreenState) {
                ...
                if (state.refreshButton == true)
                  Positioned(
                    ...
                    child: FloatingActionButton(
                      ...
                      onPressed: () {
                        context
                            .read<EditScreensCubit>()
                            .showGlobalDeckInfo(state.globalDeckInfo.globalDeck.ID,state.globalDeckInfo.globalDeck.title, state.globalDeckInfo.globalDeck.description, null);
                      },
                    ),
                  ),
              ],
