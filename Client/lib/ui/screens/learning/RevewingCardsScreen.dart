...
class ReviewingCardsScreen extends StatefulWidget {
  const ReviewingCardsScreen({Key? key}) : super(key: key);

  @override
  State<ReviewingCardsScreen> createState() => _ReviewingCardsScreenState();
}

class _ReviewingCardsScreenState extends State<ReviewingCardsScreen>
  ...
  ReviewingCardsScreenState? _currentState;
  ...
  void initState() {
    ...
    _controller.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        ...
        if (state is ReviewingCardsScreenState) {
          setState(() {
            _showAnswer = false;
  ...
  void _swipeLeft(RecallQuality quality) {
    ...
    context.read<LearningScreensCubit>().submitReviewingCardReview(quality);
  }
  ...
  void _startSwipeAnimation() {
    ...
    if (direction > 0) {
      context.read<LearningScreensCubit>().submitReviewingCardReview(RecallQuality.perfect);
    } else {
      context.read<LearningScreensCubit>().submitReviewingCardReview(RecallQuality.bad);
    }
  }
