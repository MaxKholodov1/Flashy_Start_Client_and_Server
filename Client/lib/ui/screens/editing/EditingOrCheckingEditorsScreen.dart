...
import '../../widgets/editing/EditingEditorsWidget.dart';

class EditingOrCheckingEditorsScreen extends StatefulWidget {
  const EditingOrCheckingEditorsScreen({super.key});

  @override
  State<EditingOrCheckingEditorsScreen> createState() => _EditingOrCheckingEditorsScreenState();
}

class _EditingOrCheckingEditorsScreenState extends State<EditingOrCheckingEditorsScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (!_snackbarShown &&
        state is EditingEditorsScreenState &&
  ...
  Widget build(BuildContext context) {
    ...
    final authorID = (context.read<EditScreensCubit>().state as EditingEditorsScreenState).globalDeckInfo.globalDeck.authorID;
        ...
        return Scaffold(
          body: BlocListener<EditScreensCubit, EditingScreenState>(
            listener: (context, listenerState) {
              if (listenerState is EditingEditorsScreenState &&
                        ...
                        return SingleChildScrollView(
                          ...
                          child: ConstrainedBox(
                            ...
                            child: Center(
                              child:
                              isAuthor
                               ? EditingEditorsWidget(
                                author: state.globalDeckInfo.authorName,
                                editors:  state.globalDeckInfo.editorNames,
                              )
                              : EditorsWidget(editors: state.globalDeckInfo.editorNames, authorName: state.globalDeckInfo.authorName, ),
                            ),
                          ),
                        );
                      },
                ...
                if (state.refreshButton == true)
                  Positioned(
                    ...
                    child: FloatingActionButton(
                      ...
                      onPressed: () {
                        // TODO: возможно, обновить редакторов
                      },
                    ),
                  ),
              ],
